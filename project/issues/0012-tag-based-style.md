...
東京メトロの路線カラーを参考にした推奨色を提供する。これらは `getComputedItemStyle()` がデフォルトとして適用するが、タグ・ルール・手動指定で上書き可能。

| type | 色 | 路線イメージ |
|---|---|---|
| `task` | `#009bbf` | 東西線スカイ |
| `appointment` | `#c1a470` | 有楽町線ゴールド |
| `deadline` | `#e85298` | 丸ノ内線ピンク |

### プリセットルール（Phase 1 で実装）

**期限超過タスク**（`overdue-task`）
- 条件: `type === 'task'` かつ `status !== 'done'` かつ `getSpanEnd(temporal)` が現在時刻より前
- 適用スタイル: `{ borderColor: '#ff0000', borderWidth: '3px' }`
- 意図: 対処が必要なアイテムを赤枠で視覚的に警告する
...
- [ ] `src/lib/utils/itemUtils.ts` — `getComputedItemStyle()` 関数を追加（タイプ別デフォルト → タグスタイル → ルール → 手動の優先度でマージ）
- [ ] `src/lib/utils/itemUtils.ts` — `METRO_COLOR_PRESETS` 定数を定義（type別デフォルトカラー）
- [ ] `src/lib/utils/itemUtils.ts` — `PRESET_STYLE_RULES` 定数を定義（期限超過・完了済みのプリセットルール）
- [ ] `src/lib/components/CalendarView.svelte` — `tagStyleMap` prop を追加し WeekView / MonthView に渡す
- [ ] `src/lib/components/WeekView.svelte` — アイテム描画時に `getComputedItemStyle()` を使用するよう切り替え
- [ ] `src/lib/components/MonthView.svelte` — アイテム描画時に `getComputedItemStyle()` を使用するよう切り替え
- [ ] `src/index.ts` — `getComputedItemStyle` / `METRO_COLOR_PRESETS` / `PRESET_STYLE_RULES` をエクスポート
- [ ] `tests/unit/itemUtils.test.ts` — `getComputedItemStyle()` のユニットテスト（優先度・各ルール・タグマッピングの動作）
- [ ] `tests/e2e/tag-style.spec.ts` — E2E テスト（タグ追加時のスタイル適用・期限超過アイテムの赤枠・完了タスクのグレーアウト）
- [ ] `demo/App.svelte` — `tagStyleMap` の使用例をデモに追加
