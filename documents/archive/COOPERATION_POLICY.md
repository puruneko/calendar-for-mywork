...
## 0. session contextに刻み、必ず厳守するキーワード
- **コメントおよび説明文は日本語で記述すること**
- テストは仕様／コードが変わるたび内容を見直し、常にメンテナンスすること
- クレジット消費を最小限に抑えること
- 私へのレスポンスは必要最低限とする
...
2. **documents配下のMarkdownファイル**
   - `agent/COOPERATION_POLICY.md` (本ファイル) - 開発ルール
   - `agent/ONBOARDING.md` - 引き継ぎ情報と現在の状態
   - `agent/PROJECT_POLICY.md` - プロジェクトの意思・仕様・作業状況」の管理
   - `ARCHITECTURE.md` - 技術的なアーキテクチャ説明
   - `LIBRARY_USAGE.md` - ライブラリ利用者向けドキュメント

### 2.2 ONBOARDING.mdの必須確認
- **ONBOARDING.mdは必須確認**
- 記載されている**引き継ぎチェックリストをすべて完了**させること（この時ファイル内容は変更しない）
...
**UI/UX関連の変更は、E2Eテストのパスだけでなく、必ずブラウザで実際の動作を確認・開発コンソールを確認すること**
